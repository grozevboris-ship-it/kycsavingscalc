...
import {
  Clock, ArrowRight, MessageSquareOff,
        ...
        <div className="lg:col-span-4 space-y-4">
          <Card className="border-slate-200">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div className="space-y-1">
                  <span className="text-[10px] font-bold text-slate-400 uppercase">Audits/Year</span>
                  <Input type="number" className="h-8 text-sm" value={annualAudits} onChange={(e) => setAnnualAudits(Number(e.target.value))} />
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="space-y-2">
             {[
               ...
               <div key={p.id} className={`p-3 rounded-lg border transition-all ${selectedProducts[p.id as keyof typeof selectedProducts] ? `bg-white ${p.color} shadow-sm` : 'bg-slate-50 border-slate-200 opacity-60'}`}>
                <div className="flex items-start gap-3">
                  <Checkbox id={p.id} checked={selectedProducts[p.id as keyof typeof selectedProducts]} onCheckedChange={(c) => setSelectedProducts({...selectedProducts, [p.id]: !!c})} className="mt-1" />
                  <div className="space-y-1">
